...
function wpblog_load_textdomain() {
	load_plugin_textdomain( 'blog-location', false, dirname( plugin_basename( __FILE__ ) ) . '/languages' );
}
...
function wpblog_post_add_settings_page() {
    add_options_page(
        __( 'IP Location Settings', 'blog-location' ), // Page title
        __( 'IP Location', 'blog-location' ), // Menu name
        'manage_options', // User capability
        'blog-location', // Page ID
    ...
    add_settings_field(
        ...
        __( 'Show author location on post pages', 'blog-location' ), // Field title
...
function wpblog_post_settings_page() {
    ...
    if ( ! current_user_can( 'manage_options' ) ) {
        wp_die( esc_html__( 'You do not have sufficient permissions to access this page.', 'blog-location' ) );
    }
    ...
    if ( isset( $_POST['wpblog_post_save_settings'] ) && isset($_POST['_wpnonce']) && wp_verify_nonce($_POST['_wpnonce'], $nonce) ) {
        ...
        echo '<div id="setting-error-settings_updated" class="updated settings-error notice is-dismissible"><p><strong>' . esc_html__( 'Settings saved.', 'blog-location' ) . '</strong></p></div>';
    }
...
<div class="wrap">
    ...
    <h2><?php esc_html_e( 'WordPress Blog User IP address attribution', 'blog-location' ); ?></h2>
    <p><?php esc_html_e( '1. Display WordPress user IP address attribution and city location information, More information at', 'blog-location' ); ?> <a href="https://wpblog.cn" target="_blank" rel="noopener">WPblog.cn</a></p>
    <p><?php esc_html_e( '2. You can display the author or publisher location anywhere on your website. The shortcode is', 'blog-location' ); ?> <code>[wpblog_post_location]</code> <code>[wpblog_author_location]</code> </p>
    <form method="post" action="">
        <input type="hidden" name="_wpnonce" value="<?php echo $nonceStr; ?>"/>
        <table class="form-table">
            <tbody>
                <tr>
                    <th scope="row"><?php esc_html_e( 'Post/pages', 'blog-location' ); ?></th>
                    <td><label><input type="checkbox" name="show_post_location" value="1" <?php checked( $show_post_location, true ); ?>> <?php esc_html_e( 'Show location', 'blog-location' ); ?></label></td>
                </tr>
                <tr>
                    <th scope="row"><?php esc_html_e( 'Comments', 'blog-location' ); ?></th>
                    <td><label><input type="checkbox" name="show_comment_location" value="1" <?php checked( $show_comment_location, true ); ?>> <?php esc_html_e( 'Show location', 'blog-location' ); ?></label></td>
                </tr>
                <tr>
                    <th scope="row"><?php esc_html_e( 'IPChannel', 'blog-location' ); ?></th>
                    <td>
                    <?php foreach ($allowIpCheckerArr as $checker) : ?>
                        <label style="margin-right: 20px;">
                            ...
                            <?php esc_html_e('IPChannel-' . $checker, 'blog-location'); ?>
                        </label>
                    <?php endforeach; ?>
                    </td>
                </tr>
                <tr>
                    <th scope="row"><?php esc_html_e( 'IPAddressFormat', 'blog-location' ); ?></th>
                    <td>
                        <fieldset>
                            <?php
                            ...
                            foreach ( $addressFormatArr as $format ) {
                                ...
                                esc_html_e( $format, 'blog-location' );
                ...
                <tr>
                    <th scope="row"><?php esc_html_e( 'IPAddressCustomForAdmin', 'blog-location' ); ?></th>
                    <td>
                        <fieldset>
                            <?php
                            ...
                            echo ' /> <span class="date-time-text format-i18n">';
                            esc_html_e( WpBlogConst::WPBLOG_POST_DEFAULT_FALSE, 'blog-location' );
                            echo "</span></label><br />\n";
                            ...
                            echo "<code>";
                            esc_html_e("ip_address_custom_for_admin_tips", 'blog-location');
                            echo "</code><br />";
                            ?>
                        </fieldset>
                    </td>
        ...
        <?php submit_button( __( 'Save Changes', 'blog-location' ), 'primary', 'wpblog_post_save_settings' ); ?>
    </form>
</div>
<?php
}
